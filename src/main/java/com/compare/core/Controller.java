...
import com.compare.user_interface.OverwriteOption;
...
public class Controller {
    ...
    private static final String DATA_REP_INTERFACE_PATH = "com.compare.data_representation.DataRepresentation";
    private static final String PAIRWISE_COMPARISON_INTERFACE_PATH = "com.compare.metrics.comparison.PairwiseComparisonStrategy";
    private static final String AGGREGATION_INTERFACE_PATH = "com.compare.metrics.aggregation.AggregationStrategy";
    ...
    @Autowired
    private List<PairwiseComparisonStrategy> listOfImpls = new ArrayList<>();
    @Autowired
...
//    public static Controller getController(){
        // TODO: This code needs to go into the file reader for reading the config
...
//    }
    ...
    private void processCompareCommand(CompareDTO dto) {
        ...
        String dataRepresentationName = dto.getDataRepresentation();
        if (dataRepresentationName == null) {
            dataRepresentationName = config.getDataRepresentation();
        }
        DataRepresentation dataRepresentation = context.getBean(dataRepresentationName, DataRepresentation.class);
        if(dataRepresentation == null)
            return;

        //load the pairwise comparison metric
        String pairwiseMethod = dto.getPairwiseMethod(); //TODO: Have this return an Optional so that we can say .orElse(config.getComparisonMethod());
        if (pairwiseMethod == null)
            pairwiseMethod = config.getComparisonMethod();

        PairwiseComparisonStrategy comparisonStrategy = context.getBean(pairwiseMethod, PairwiseComparisonStrategy.class);
        if(comparisonStrategy == null)
            return;

        //load the aggregation method
        String aggregationStrategyName = dto.getAggregationMethod();

        if (aggregationStrategyName == null)//nothing specified in dto, so load default from config file
            aggregationStrategyName = config.getAggregationMethod();
        AggregationStrategy aggregationStrategy = context.getBean(aggregationStrategyName, AggregationStrategy.class);
...
//            comparisonService = new ComparisonService(dto.getNumberOfThreads()); TODO
        String result = null;
        try {
    ...
    private void processHelpCommand(HelpDTO dto) {
        ...
        switch(helpType){
            case Command:
                ...
                result.append("\t\t\t-m <metric>: set the diversity metric to use in the calculation. Available com.compare.metrics can be found with 'help -m'\n");
                ...
                result.append("\t\t\t-m: lists the available comparison com.compare.metrics in the system\n");
        ...
        /* for comparison com.compare.metrics, aggregation methods, and data representations, we search for every
...
//            Object[] objects = reflectionService.searchPackage(packageName, interfacePath);
            ...
            interfacePath = interfacePath.substring(0, interfacePath.length() - ".class".length());
            if (interfacePath.substring(interfacePath.lastIndexOf('.'), interfacePath.length()).equalsIgnoreCase("pairwiseComparisonStrategy"))
                objects = listOfImpls.toArray();
